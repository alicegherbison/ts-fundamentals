...
myAny.foo.bar.baz;
myUnknown.foo;
// a good use of unknown is an api reponse - performing an ascertion to check it is what you want it to be
...
// make sure there is only one place in your code that is doing branding and unbranding
...
if (typeof x === "string") {
  ...
  // x is a never here - you're not going to be creating values of type 'never' but this is where you would see them
}
...
// if you add boolean, because of the last clause below you are asserting that y must be a never by this time e.g. you have unhandled cases - you need to handle them - this is an exhaustive conditional / switch. could be a switch too. handle everything before reaching last clause - in a switch this would be the default clause.
